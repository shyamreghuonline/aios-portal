...
export default function StudentEditPage() {
  ...
  const urlStudentId = typeof window !== 'undefined' ? window.location.pathname.split('/').slice(-2)[0] : '';
  const canEdit = true; // Admin always can edit
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const snap = await getDoc(doc(db, "students", urlStudentId));
        if (snap.exists()) {
          const d = snap.data();
          setStudent(d);

          // Set all student data fields
          if (d.totalFee) setTotalFee(d.totalFee as number);
      ...
      try {
        const q = query(collection(db, "payments"), where("studentPhone", "==", urlStudentId), orderBy("createdAt", "desc"));
  ...
  async function handlePhotoUpload(file: File) {
    ...
    try {
      ...
      const studentRef = doc(db, "students", urlStudentId);
  ...
  async function savePersonal() {
    ...
    try {
      ...
      await setDoc(doc(db, "students", urlStudentId), payload, { merge: true });
  ...
  async function saveAcademic() {
    ...
    try {
      await setDoc(doc(db, "students", urlStudentId), { academicDetails: cleanObject(academic) }, { merge: true });
